...
options(scipen=999)
    ...
    #suppressMessages({                         
                        ...
                        # 1.2 Benchmark revenues -----
                           ...
                           # Benchmark_tax_rate <- Benchmark_tax_rate
                            ...
                            CPA_PRODUCTS$BM_Rev$Final_Demand_HH[grepl("Real estate services", CPA_PRODUCTS$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
                            ...
                            CPA_PRODUCTS$BM_Rev$Final_Demand_NPISH[grepl("Real estate services", CPA_PRODUCTS$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
                            ...
                            CPA_PRODUCTS$BM_Rev$Final_Demand_Government[grepl("Real estate services", CPA_PRODUCTS$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
  ...
  # New 22/09/2023      
                              ...
                              CPA_PRODUCTS_BU$Est_Rev$Final_Demand_HH[CPA_PRODUCTS_BU$Est_Rev$PRODUCT_INDUSTRY_NAME == "Construction services"] <- 
                                CPA_PRODUCTS_BU$Use_K_NetPurch$Final_consumption_expenditure_by_households[CPA_PRODUCTS_BU$Use_K_NetPurch$PRODUCT_INDUSTRY_NAME == "Construction services"]*
              ...
              # 2. SIMULATION RESULTS ----  
                ...
                # 3.PREPARATION TABLE FOR EXPORT AND PLOTTING ------------------------------------------
                          ...
                          # 3.2 Main results --------------------------------------------------------
                              ...
                              # Rounding 
                              Export_Main_Results_SIM$value<-round(Export_Main_Results_SIM$value,2)
                            ...
                            # Assuming your data frame is named Export_Main_Results
                              ...
                              # Total_VAT_Gap
                                ...
                                # Policy_Gap_Pct
                                        ...
                                        Export_Main_Results<-Export_Main_Results%>%
                                        dplyr::mutate(Difference= Simulation-Actual,
                                                      "Simulation as % of GDP"=((abs(Difference)/GDP_2018*100)),
                                                      Pct_Change=((Simulation/Actual)*100-100)
                                        )
                              ...
                              Export_Main_Results$Simulation<-round(Export_Main_Results$Simulation,2)
                              Export_Main_Results$`Difference(Simulation-Actual)`<-round(Export_Main_Results$`Difference(Simulation-Actual)`,2)
                              Export_Main_Results$`Difference  as % of GDP`<-round(Export_Main_Results$`Difference  as % of GDP`,2)
                              Export_Main_Results$`Pct_Change(Simulation/Actual)`<-round(Export_Main_Results$`Pct_Change(Simulation/Actual)`,2)
